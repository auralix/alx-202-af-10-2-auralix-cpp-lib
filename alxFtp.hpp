	...
	namespace AlxFtp
	{
		...
		class IFtp
		{
			public:
				...
				virtual Alx_Status Client_ChangeDir(const char* path, bool* exists) = 0;
		...
		class Ftp : public IFtp
		{
			public:
				...
				Alx_Status Client_ChangeDir(const char* path, bool* exists) override
				{
					return AlxFtp_Client_ChangeDir(&me, path, exists);
				}
